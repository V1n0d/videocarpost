(function($) {
	var	aux		= {
			...
			navigate	: function( dir, $el, $wrapper, opts, cache ) {
				...
				if( cache.expanded ) {
					scroll		= 4; // scroll is always 1 in full mode
			...
			init 		: function( options ) {
				...
				if( this.length ) {
					...
					return this.each(function() {
						...
						var $navPrev		= $('.scroll-videos .glyphicon-chevron-left'),
							$navNext		= $('.scroll-videos .glyphicon-chevron-right');
						...
						// navigate left
						$navPrev.on('click.contentcarousel', function( event ) {
							if( cache.isAnimating ) return false;
							cache.isAnimating	= true;
						...
						// navigate right
						$navNext.on('click.contentcarousel', function( event ) {
							if( cache.isAnimating ) return false;
							cache.isAnimating	= true;
							aux.navigate( 1, $el, $wrapper, settings, cache );
						});
						...
						$el.on('mousewheel.contentcarousel', function(e, delta) {
							/*if(delta > 0) {
								if( cache.isAnimating ) return false;
								cache.isAnimating	= true;
								aux.navigate( -1, $el, $wrapper, settings, cache );
